...
export default function Chargers() {
  ...
  const rows = useMemo(() => {
    ...
    return s
      ? data.filter((r) =>
          `${r.site_name} ${r.operator_name} ${r.city ?? ''} ${r.charge_point_name}`
            .toLowerCase()
            .includes(s),
        )
      : data;
  ...
  return (
    <Screen>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          <Body muted style={{ fontSize: 12 }}>
            {coords === undefined
              ? 'Finding you…'
              : coords === null
                ? 'Location off · showing all sites'
                : `${rows.length} chargers near you`}
          </Body>
      ...
      <FlatList
        ...
        refreshControl={
          <RefreshControl refreshing={isRefetching} onRefresh={refetch} tintColor={C.green} />
        }
        ListEmptyComponent={
          !isLoading ? (
            <Empty>
              {coords === undefined
                ? ''
                : 'No chargers found. Pull to refresh, or widen your search.'}
            </Empty>
          ) : null
        }
...
function ChargerRow({ r }: { r: NearbyCharger }) {
  ...
  return (
    <Pressable
      onPress={() => router.push(`/charger/${r.charge_point_id}`)}
      style={({ pressed }) => ({
        backgroundColor: C.white,
        borderRadius: 16,
        borderWidth: 1,
      ...
      <View style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 12 }}>
        <View
          style={{
            ...
            height: 40,
            borderRadius: 10,
            backgroundColor: online ? C.honeydew : C.divider,
            alignItems: 'center',
        ...
        <View style={{ alignItems: 'flex-end', gap: 4 }}>
          <Badge
            status={online ? (available > 0 ? 'Available' : 'Charging') : 'Offline'}
            label={online ? `${available}/${r.connectors.length} free` : 'Offline'}
          />
          {r.tariff_text && (
            <Text style={{ fontSize: 12, fontWeight: '700', color: C.green }}>{r.tariff_text}</Text>
          )}
        </View>
      </View>
