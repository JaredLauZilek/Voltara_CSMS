...
export function useSessionCdr(sessionId: string, enabled: boolean) {
  return useQuery({
    queryKey: ['sessions', 'cdr', sessionId],
    queryFn: () => api.getCdr(sessionId),
    enabled,
...
export function useReceipt(cdrId: string | null) {
  return useQuery({
    queryKey: ['sessions', 'receipt', cdrId],
    queryFn: () => api.getReceipt(cdrId!),
    enabled: Boolean(cdrId),
  });
}

export function useSessionMinutes(sessionId: string, live: boolean) {
  return useQuery({
    queryKey: ['sessions', 'minutes', sessionId],
    queryFn: () => api.sessionMinutes(sessionId),
