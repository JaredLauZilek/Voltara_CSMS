...
export async function saveProfile(userId: string, patch: Partial<Pick<DriverProfile, 'display_name' | 'phone' | 'vehicle_plate' | 'vehicle_model'>>): Promise<DriverProfile> {
  const { data, error } = await supabase.from('driver_profiles').upsert({ user_id: userId, ...patch }).select().single();
  if (error) throw error;
  return data;
}
...
export async function listMyTags() {
  const { data, error } = await supabase.from('id_tags').select('id, tag, label, status, tenant_id, created_at').order('created_at');
  if (error) throw error;
  return data ?? [];
}
