...
- [x] **Phase 3 — Tariffs, billing math, CDRs, reports** · code complete 21 Sep 2026 — OCPI-shaped tariffs with immutable versions and scoped assignments, driver groups, billing accounts (MyInvois fields), tax profiles, cost engine (sen-exact, property-tested), gateway snapshot at start + CDR at stop + idle tracking + signed webhooks, charging records with CSV export, receipts / invoice runs / settlement statements assembled in SQL, revenue reports, session simulator + nightly canary. Design: [docs/phase3-billing-design.md](phase3-billing-design.md) · operator test script: [docs/testing-guide-phase3.md](testing-guide-phase3.md). Open: Voltara's real tariffs as golden cases; SST rate confirmation; CANARY_* secrets to switch the canary on.
